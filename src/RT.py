...
def effects_due_to_gravity(ds, recom=False):
...
def effects_due_to_winds(ds, wind="zon", recom=False):

    gamma = ds["ratio"] * (-ds[wind] + (ds["ge"] / ds["nui"])) * ds["K"]
...
def all_effects(ds, wind="zon", sign_wd=-1, drift="vz", recom=False):

    gamma = (
        ds["ratio"]
        * (ds[drift] + sign_wd * ds[wind] + (ds["ge"] / ds["nui"]))
        * ds["K"]
    )
...
def effects_due_to_drift(ds, col="vz", recom=False):

    gamma = ds["ratio"] * (ds[col] + (ds["ge"] / ds["nui"])) * ds["K"]
